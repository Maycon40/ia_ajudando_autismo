...
export default function CharacteristicsForm() {
  const {
    responses,
    setResponses,
    questionsFormCharacteristics,
    setQuestionsFormCharacteristics,
  } = useAuthContext();
  ...
  const handleSubmit = async (event) => {
    ...
    for (let question of questionsFormCharacteristics) {
      ...
      if (question["type"] == "textarea") {
        question["responseGemini"] = await getResponseGemini(
          question,
  ...
  return (
    <form onSubmit={handleSubmit} className="container mx-auto p-4">
      <p className="font-bold mb-6">
        Aviso: Esse questionário não substitui a avaliação de um profissional,
        mas pode auxiliar na identificação de características do autismo.
      </p>
